window.addEventListener("DOMContentLoaded", async function () {
  ...
  async function loadExcelData() {
    try {
      ...
      if (rows.length === 0) {
        throw new Error("O CSV está vazio ou inválido.");
      }

      headers = rows
        .shift()
        .map((header) => header.toUpperCase().trim());
  ...
  function findOrderRows(orderNumber) {
    ...
    return tableData.filter(
      (row) => normalize(row[orderKey]) === normalizedOrder
    );
  }
  ...
  function updateResultScreen() {
    ...
    resultProgress.textContent =
      `${currentIndex + 1}/${currentOrderRows.length}`;

    resultLocation.value = "";
    resultLocation.placeholder =
      row[locationKey] || "Endereço não informado";

    resultSku.value = "";
    resultSku.placeholder =
      row[skuKey] || "SKU não informado";
  ...
  function validateLocation() {
    ...
    if (enteredLocation !== expectedLocation) {
      ...
      showErrorPopup(
        "Localização incorreta. Verifique e tente novamente."
      );
  ...
  function validateItem() {
    ...
    if (enteredSku !== expectedSku) {
      ...
      showErrorPopup(
        "Item incorreto. Verifique e tente novamente."
      );
